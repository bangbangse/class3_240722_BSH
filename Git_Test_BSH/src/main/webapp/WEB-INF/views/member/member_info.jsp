	...
	<article>
		...
		<form action="MemberModify" name="joinForm" method="post">
			<table border="1">
				<tr>
					<th>이름</th>
					<td><input type="text" name="name" id="name" value="${member.name}" pattern="^[가-힣]{2,6}$" title="한글 2~6글자" required></td>
				</tr>
				<tr>
					<th>ID</th>
					<td>
				...
				<tr>
					...
					<td>
						<select name="job">
							...
							<%-- c:if 태그를 사용하여 직업이 XXX 일 경우 selected 속성 추가 --%>
				...
				<tr>
					...
					<td>
						...
						<%-- split() 함수를 통해 분리 후 판별하거나 contains() 함수를 통해 분리 없이 문자열 검색 --%>
