...
<!-- 						<div> -->
							<%-- 
							파일 다운로드 기능을 추가하려면 하이퍼링크에 download 속성 설정 추가시 다운로드 가능
...
<!-- 							</a> -->
							...
							<%-- 1) split() 함수 활용하여 "_" 구분자로 분리 후 두번째 인덱스값 사용--%>
							<%--	단, _ 기호가 2개 이상을 겨우 반복문을 통해 문자열 결합 추가 필요 --%>
<%-- 							split() : ${fn:split(board.board_file1, "_")[1]} --%>
							
							<%-- 2) substring() 함수 활용하여 시작인덱스부터 지정인덱스까지 추출 --%>
							<%-- 2-1) 파일명의 길이 알아내기 --%>
<%-- 							<c:set var="fileLength" value="${fn:length(board.board_file1)}"></c:set> --%>
							<%-- 2-2) 구분자(_)의 인덱스 알아내기(복수개일 경우 첫번째 기호 탐색) --%>
<%-- 							<c:set var="delimIndex" value="${fn:indexOf(board.board_file1, '_')}"></c:set> --%>
...
<%-- 						<div>${board.board_file3}</div> --%>
						...
						<c:forEach var="file" items="${fileList}" varStatus="status">
							...
							<c:if test="${not empty file}">
								<div>
									${file}
	 								<a href="${pageContext.request.contextPath}/resources/upload/${board.board_file1}" download="${originalFileList[status.index]}">
	 								<input type="button" value="다운로드">
	 								</a>
 								</div>
							</c:if>
	...
	<script type="text/javascript">
		...
		function getQueryParams() {
			...
			if (params.lastIndexOf("&" == params.length - 1)) {	// & 기호가 배열의 끝에 있을경우
		...
		function confirmDelete() {
			if(confirm("삭제하시겠습니까?")){
				location.href = "BoardDelete?" + getQueryParams();
			}
		}
		
		function requestModify() {
			location.href = "BoardModify?" + getQueryParams();
		}
