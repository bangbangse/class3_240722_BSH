...
<body>
	...
	<article>
					...
					<c:otherwise>
						...
						<c:forEach items="${boardList}" var="board">
							...
							<tr>
								...
								<td>
									<%--
									JSTL - format(fmt) 라이브러리를 화룡하여 날짜 및 시각 형식(포맷) 변경
									1) <fmt:formatDate> : Date등의 날짜 및 포맷 객체의 날짜 형식 변경
		...
		<section id="pageList">
			...
			<c:set var="keywordParam" value="&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}"></c:set>
			...
			<%-- 시작 페이지가 1페이지일 경우 비활성화(disabled) --%>
			...
			>
			<%-- [이전] 버튼 클릭 시 이전 페이지 글목록 요청(파라미터로 현재페이지 - 1)  --%>
			<%-- 현재 페이지가 1 페이지일 경우 비활성화(disabled) --%>
			<input type="button" value="&lt;" onclick="location.href='BoardList?pageNum=${pageNum - 1}<c:if test="${not empty param.searchKeyword}">${keywordParam}</c:if>'"
			...
			<input type="button" value="&gt;&gt;" onclick="location.href='BoardList?pageNum=${pageInfo.startPage + pageInfo.pageListLimit}<c:if test="${not empty param.searchKeyword}">${keywordParam}</c:if>'"
				<c:if test="${endpage == pageInfo.maxPage}">disabled</c:if>
			>
		</section>
	</article>
...
// 			console.log(parent.html());		//	부모요소 내의 HTML 태그 모두 출력됨
			...
			//	부모 요소의 내부 요소들 중에서 클래스 선택자 "board_num" 요소 탐색 = find() 메서드 활용
