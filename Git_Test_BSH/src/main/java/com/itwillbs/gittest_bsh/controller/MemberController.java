...
import com.itwillbs.gittest_bsh.service.MemberService;
...
//		System.out.println(member);
		 ...
		 * - org.springframwork.security.crypto.bcrypt.BcryptPasswordEncoder 클래스 활용
...
//		BCryptPasswordEncoder passwordEncoder = new BCryptPasswordEncoder();
		...
		System.out.println("암호문 : " + securePasswd);		//	$2a$10$Y2iruQcLb2ZCwinMBLyfo.kQtUmz1xNs7/AjFbWPlY34W563dXd1m
	...
	public String memberLogin(MemberVO member, String rememberId, HttpSession session, Model model, BCryptPasswordEncoder passwordEncoder) {
		
		//	[ BCryptPasswordEncoder 클래스를 활용하여 암호화 된 패스워드 비교 작업을 위한 패스워드 조회 ]
		//	MemberService - getMember() 메서드 호출하여 회원 상세정보 조회 요청(회원 상세정보 재사용)
		//	=> 파라미터 : MemberVO 객체(id 필요)	리턴타입 : MemberVO(dbMember)	// dbMember => 기존과 다른 변수
...
//		System.out.println("입력받은 패스워드 : " + member.getPasswd());
		 ...
		 */
		//	패스워드만 조회했을 경우
//		if (dbPasswd == null || !passwordEncoder.matches(member.getPasswd(), dbPasswd)) {
//		}
		//	회원 상세정보를 조회했을 경우
		//	1) 로그인 성공 실패 판별
		//	2) 회원 상태(휴면(생략)/탈퇴) 판별
		if (dbMember == null || !passwordEncoder.matches(member.getPasswd(), dbMember.getPasswd())) {
			...
			return "result/fail";
		} else if (dbMember.getMember_status() == 3) {	// 로그인 성공이지만, 탈퇴회원일 경우
			model.addAttribute("msg", "탈퇴한 회원입니다!");
			return "result/fail";
		} else {	// 로그인 성공
			//	로그인 성공한 아이디를 세션에 저장(속성명 "sId")
			session.setAttribute("sId", member.getId());
			//	세션 타이머 1시간으로 변경(클라이언트로 부터 1시간동안 요청이 없을 경우 세션 제거됨)
			...
			//	  null 일 경우(로그인 링크 눌렀을 때) 기존과 동일하게 메이페이지로 리다이렉트
			if (session.getAttribute("prevURL") == null) {
				return "redirect:/";	// 메인페이지로 리다이렉트
				
			} else {
				//	request.getServletPath() 메서드를 통해 이전 요청 URL 을 저장할 경우
				//	"/요청URL" 형식으로 저장되므로 redirect:/ 에서 
...
//			System.out.println("queryString : " + queryString);
		...
		//	=> 파라미터 : MemberVO 객체(아이디만 전달해도 무관함)	리턴타입 : MemberVO(기존 변수 = member 재사용)
		//	=> 이 때, 아이디 파라미터가 없으므로 세션 아이디 값을 MemberVO 객체에 저장
		//	=> 조회결과 저장 시 새로운 MemberVO 타입 변수 선언 대신 기존 변수 member 재사용 가능
...
//	public String modify(MemberVO member, String oldPasswd, Model model, HttpSession session) {
//	public String modify(@RequestParam Map<String, String> map, MemberVO member, Model model, HttpSession session, BCryptPasswordEncoder passwordEncoder) {
	
	public String modify(@RequestParam Map<String, String> map, String[] hobby, Model model, HttpSession session, BCryptPasswordEncoder passwordEncoder, HttpServletRequest request) {
		//	만약, 파라미터 매핑용 매개변수를 Map 타입을 선언 시 @RequestParam 필수!
		//	만약, Map 타입 파라미터와 MemberVO 타입 파라미터를 동시에 선언 시 두 객체 모두 파라미터 저장
...
//			System.out.println("취미 : " + map.get("hobby"));
		...
		//		=> success.jsp와 fail.jsp 파일을 하나로 통합하여 처리해도 가능
		//	=>	실패 시 "회원정보 수정 실패!" 메세지 저장후 이전 페이지 처리(fail.jsp)
		if (updateCount > 0) {
...
//			System.out.println("queryString : " + queryString);
	...
	public String withdraw(String passwd,BCryptPasswordEncoder passwordEncoder ,HttpSession session ,Model model) {
		//	입력받은 패스워드 확인
		String id = (String)session.getAttribute("sId");
